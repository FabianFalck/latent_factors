...
mfd <- function(# primary (necessary) inputs
          ...
          # tertiary inputs
          seed = 1, loadings.rotation = "promax") {
  ...
  object_rmNA<-remove_all_NA_rows(Y_loadings, Y_scores, data_loadings, data_scores)
  Y_loadings<-object_rmNA[[1]]
  Y_scores<-object_rmNA[[2]]
  data_loadings<-object_rmNA[[3]]
  data_scores<-object_rmNA[[4]]
  ...
  # list[A, ppca_object] <- 
  object_estimate<-  estimate_loadings(Y_in = Y_loadings,
                                            type = "n_PCs",
                                            var_explained = NULL,
  ...
  # list[Z, S] <-
  object_scores <-estimate_scores(Y_in = Y_scores, A_in = A, type = scores.type)  # type is "a_posteriori" OR "lin_reg"
  Z<-object_scores[[1]]
  S<-object_scores[[2]]
...
normalize_loadings <- function(mat, type = "largest_abs") {
  ...
  if (type == "largest_abs") {
    ...
    mat <- t(t(mat) / apply(mat, 2, function(v) max(abs(v), na.rm = T)))  # function t() returns transpose; na.rm means missing values are ignored
...
estimate_scores <- function(Y_in, A_in, type = "lin_reg") {
  ...
  # list[unique_missing_patterns_char, indices_Y_in_list] <-
  data_tmp<-  find_unique_missing_patterns(Y_in)
  unique_missing_patterns_char<-data_tmp[[1]]
  indices_Y_in_list<-data_tmp[[2]]
