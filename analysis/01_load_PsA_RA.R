...
d0_RA<- read.csv(file.path("/data/as/processed/clinical/database_views/","v_proj1_RA_filtered_20220125.csv"))
d0_PsA<- read.csv(file.path("/data/output/wranglers/IL17/project_1/database_views/PsA/","v_proj1_PsA_20211117.csv"))
...
## for RA new Release
d1_RA<-d0_RA %>% mutate(PARAMCD2=ifelse(PARAMCD=="RADA01","PADA01",ifelse(PARAMCD=="RADA02","PADA02",ifelse(PARAMCD=="RAP01","PAP01",PARAMCD))) ) %>% select(-PARAMCD) %>% dplyr::rename(PARAMCD=PARAMCD2) %>% 
...
d1_PsA<-d0_PsA %>% mutate(PARAMCD2=ifelse(PARAMCD=="RADA01","PADA01",ifelse(PARAMCD=="RADA02","PADA02",ifelse(PARAMCD=="RAP01","PAP01",ifelse(PARAMCD=="PAP02","PAP01",PARAMCD)))) ) %>% select(-PARAMCD) %>% dplyr::rename(PARAMCD=PARAMCD2) %>% 
  mutate(TRT01P=ifelse(STUDYID=="CAIN457F2208",TRT01A,TRT01P)) %>% 
