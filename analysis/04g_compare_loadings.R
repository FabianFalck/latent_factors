...
plot_num <- 0
layout(mat = matrix(c(1, 2, 3, 4, 1, 5, 6, 7, 1, 8, 9, 10, 1, 11, 12, 13), nrow = 4, ncol = n_fac + 1, byrow = TRUE))
par(mar = c(2, 1, 1, 2), 
    oma = c(5, 8, 7, 2),
    mfrow = c(1, 5))

names(control$do_list)
analyses_to_plot <- c("PsA_F2312", "PsA_F2342", "RA_F2302", "RA_F2309", "RA_F2302_cross_indication_loadings")
for (analysis_plot in analyses_to_plot) {
  ...
  #            y_fac = 0.034,
  #            cex = 1.25)
  if (analysis_plot == analyses_to_plot[5]) {
...
#                   # add_legend = fac_curr == n_fac & match(study_to_plot, studies_to_plot) == 2,
#                   add_legend = fac_curr == n_fac & study_to_plot == studies_to_plot[2],
